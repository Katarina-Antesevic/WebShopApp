...
<head>
    <meta charset="UTF-8">
    <title>Izmjena specificne kategorije</title>
...
<body>
    <nav class="navbar navbar-expand-sm navbar-dark bg-secondary sticky-top">
      ...
      <form method="POST" action="?action=saveUpdatedCategory" class="form-new-Cat">
        <h2 class="form-heading">Promijenite trenutno ime specificne kategorije</h2>
        <p></p>
        <p></p>
        <input name="naziv" type="text" class="form-control" value="<%=session.getAttribute("nazivSpecKategorije")!=null?session.getAttribute("nazivSpecKategorije").toString():""%>">
         <input type="hidden" name="id" id="id" value=<%=session.getAttribute("idSpecKategorije")!=null?session.getAttribute("idSpecKategorije").toString():""%>>
         <input type="hidden" name="idKategorije" id="idKategorije" value=<%=session.getAttribute("idKategorije")!=null?session.getAttribute("idKategorije").toString():""%>>
        
        
         <p></p>
        <h5><%=session.getAttribute("notificationTen").toString()%></h5>
