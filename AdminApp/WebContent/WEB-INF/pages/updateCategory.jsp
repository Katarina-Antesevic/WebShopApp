...
<body>
    <nav class="navbar navbar-expand-sm navbar-dark bg-secondary sticky-top">
      ...
      <form method="POST" action="?action=saveUpdatedCategory" class="form-new-Cat">
        <h2 class="form-heading">Promijenite trenutno ime kategorije</h2>
        <p></p>
        <p></p>
        <input name="naziv" type="text" class="form-control" value="<%=session.getAttribute("nazivKategorije")!=null?session.getAttribute("nazivKategorije").toString():""%>">
         <input type="hidden" name="id" id="id" value=<%=session.getAttribute("idKategorije")!=null?session.getAttribute("idKategorije").toString():""%>>
        <p></p>
        
        <h5><%=session.getAttribute("notificationSix").toString()%></h5>
